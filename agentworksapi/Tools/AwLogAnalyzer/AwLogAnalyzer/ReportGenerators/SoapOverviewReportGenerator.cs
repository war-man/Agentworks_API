...
{
    public class SoapOverviewReportGenerator : ReportGenerator
    {
        public override string CreateReport(IDictionary<string, List<Record>> logFileResults, Action<string> onUpdate)
        {
            onUpdate($"Creating Soap Overview");
            var fileName = CreateTempFile("Soap Overview");
            ...
            var apiCallsList = combinedRecordsList
                .Where(x => x.IsSoap)
                .GroupBy(x => x.SoapCall);

            var resultsList = apiCallsList
                .Select(x =>
                    new
                    {
                        ApiName = x.First().SoapCall ?? x.First().MethodAction,
                        NumberOfCalls = x.Count(),
                        ElapsedTimesList = x.Select(g => g.ElapsedMilliseconds)
                    });
